...
describe("color2Fill", () => {
  ...
  it("passes a literal hex value through directly", () => {
    expect(color2Fill("#abc")).toBe("#abc");
    expect(color2Fill("#AABBCC")).toBe("#AABBCC");
  });
  it("prefers a named mapping over hex-shaped-but-named values", () => {
    expect(color2Fill("緑")).toBe("#DDF5D9");
  });
  it("returns gray for a non-hex unmapped value", () => {
    expect(color2Fill("#12")).toBe(COLOR2_DEFAULT);
    expect(color2Fill("#1234")).toBe(COLOR2_DEFAULT);
    expect(color2Fill("xyz")).toBe(COLOR2_DEFAULT);
  });
});
